...
export default function Edit({ auth, book }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        genre: book.genre ? book.genre.split(',').map(g => g.trim()) : [],
        author: book.author || '',
        isbn: book.isbn || '',
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight font-serif">Edit Book: {book.title}</h2>}
        >
            <Head title={`Edit ${book.title} - BookVault`} />
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-100 p-8">
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <InputLabel htmlFor="quantity" value="Total Quantity Owned" />
                                    <TextInput
                                        id="quantity"
                                        type="number"
                                        name="quantity"
                            ...
                            <div>
                                <InputLabel htmlFor="cover_image" value="Cover Image (Leave empty to keep existing)" />
                                <input
                                    id="cover_image"
                                    type="file"
                                    name="cover_image"
                            ...
                            <div className="flex items-center justify-end mt-4">
                                <PrimaryButton className="ml-4 bg-blue-600 hover:bg-blue-800" disabled={processing}>
                                    Update Record
                                </PrimaryButton>
                            </div>
                        </form>
